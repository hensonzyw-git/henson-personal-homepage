...
- Historical traffic stats remain independently queryable from server logs via `npm run stats:traffic`. Henson authorized and deployed the first-party tracker under D14; D15 separately backfilled cleaned pre-tracker requests into the private dashboard.
