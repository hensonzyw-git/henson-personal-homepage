...
- Current newest publication: `harness-governance-scar-tissue`, substantially rewritten on 2026-08-21 and revised again on 2026-08-22 (zh + en; English keeps `draftTranslation: true`). It relates the real Personal Agent design to Claude Code's exposed product scale, DeepSeek Harness, and the later-opened Codex harness/App Server across six layers: product/framework, runtime loop, tool boundary, context/memory, permission/HITL, and evidence/recovery. The article stays entirely on the implemented Finance path. Each layer makes Henson's judgment explicit through options, choice criteria, rejected alternatives, concrete rules, cost, and verification; stated reading time is 27 minutes. Coding Graph Loop and false-green development-harness material remain reserved for a later standalone article. Henson selected this as the sole public Harness article on 2026-08-22; the two earlier same-material pairs, `agent-harness-teardown` and `harness-failure-direction`, were removed. `npm run deploy:ecs` then passed its build and 76 checks, updated ECS, validated/reloaded Nginx, and verified the public site plus both article routes; the removed routes return 404.
- Preceding published article: `agent-eval-methodology`, 2026-08-16 (zh + en), live in both language routes, the index, sitemap, RSS, and `/llms.txt`; `npm run deploy:ecs` completed with 72 checks, Nginx validation/reload, and public zh/en route verification. Earlier: `personal-agent-phase-one` (zh + en, 2026-08-07); the related kickoff is `all-in-personal-agent` (zh + en, 2026-08-03).
