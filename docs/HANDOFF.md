...
- Current newest article: `agent-eval-methodology`, published 2026-08-16 (zh + en; English keeps the site-wide `draftTranslation: true` convention). It is live in both language routes, the index, sitemap, RSS, and `/llms.txt`; `npm run deploy:ecs` completed with 72 checks, Nginx validation/reload, and public zh/en route verification. The preceding article is `personal-agent-phase-one` (zh + en, 2026-08-07); the related kickoff is `all-in-personal-agent` (zh + en, 2026-08-03).
