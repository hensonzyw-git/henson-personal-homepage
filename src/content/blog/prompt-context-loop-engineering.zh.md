...
> → 素材完整则进入 `ready_for_draft`，生成正文并格式化话题  
> → 草稿校验通过后进入 `ready_for_upload`  
> → 进入一次性的 `upload_started` 状态，再由 Computer Use 上传照片并填写内容<br>
> → 检查图片数量、标题、正文和话题是否已经进入页面  
> → 进入 `ready_for_final_review`，停在最终发布确认页<br>
> → 我点击发布  
> → 进入 `kb_sync_pending`，记录发布结果并校验知识库页面、索引和日志<br>
> → 校验通过后进入 `archived` 并归档素材
