(() => {
  ...
  function chart(values) {
    ...
    const svg = shape('svg', { viewBox: `0 0 ${width} 220`, role: 'img', 'aria-label': '每日页面浏览量，详细数字在下方每日数据中' });
    ...
    values.forEach((row, i) => {
      ...
      bar.append(shape('title', {}, `${row.date}：${row.pv} 次浏览，${row.uv} 位访客（估算）`));
      svg.append(bar);
  ...
  window.addEventListener('resize', () => { if (data) chart(data.windows[days].daily); });
  exclusion();
  refresh();
